...
sums :: Int -> [Int]
sums i = cumsums' [1..i] 0

cumsums' :: [Int] -> Int -> [Int]
cumsums' [] _ = []
cumsums' (x:xs) prev = (prev +x):cumsums' xs (prev+x)
...
sorted [] = True
sorted (_:[]) = True
sorted (x1:x2:t) = (x1<=x2) && sorted (x2:t)
...
mymaximum bigger initial (x:xs) = mymaximum bigger newbiggest xs
  where newbiggest = if bigger x initial then x else initial
