...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = case measure a >= measure b of
  True -> a
  False -> b
...
firstHalf xs = take n xs
  where n = (length xs) - div (length xs) 2
...
powers :: Int -> Int -> [Int]
powers k mx = takeWhile ((>=) mx) $  map (k^) [0..]
...
joinToLength n xs = filter (\s -> length s == n) [x++y | x <- xs, y <- xs]
...
sumRights = sum . map (valOrZero) 
  where valOrZero (Right r) = r
        valOrZero (Left _) = 0
...
multiCompose fs inp = applyAll (reverse fs) inp
  where applyAll [] inp1 = inp1
        applyAll (f:tl) inp1 = applyAll tl $ f inp1
...
interpreter commands = run commands [] [0,0]
  where run [] outputs _ = reverse outputs
        run (x:xs) outputs current = run xs (apply_print x current outputs) (apply_move x current)
        apply_move "up" (i:j:[]) = i:j+1:[]
        apply_move "down" (i:j:[]) = i:j-1:[]
        apply_move "left" (i:j:[]) = i-1:j:[]
        apply_move "right" (i:j:[]) = i+1:j:[]
        apply_move _ loc = loc
        apply_print "printX" (i:_:[]) outputs = show i:outputs
        apply_print "printY" (_:j:[]) outputs = show j:outputs
        apply_print _ _ outputs = outputs
