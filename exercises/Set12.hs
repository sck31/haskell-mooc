...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (LNode n l) = LNode (f n) (fmap f l)
...
count p fel = foldr (\el cnt -> cnt + (oneIfTrue$ el==p)) 0 fel where
  oneIfTrue b = if b then 1 else 0
...
inBoth l r = foldr (\a b -> b ++ if elem a r then [a] else []) [] l 
...
instance Foldable List where
  ...
  foldr f acc (LNode n l) = f n (foldr f acc l)
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode n1 n2 l) = f n1 (f n2 (foldr f acc l))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node n l r) = mconcat [sumTree l, n, sumTree r]
