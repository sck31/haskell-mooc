...
cull _ Empty = Empty
cull val (Node v l r) | v == val = Empty
...
set [] val tree = case tree of
  Empty -> Empty
  Node _ l r -> Node val l r
set _ _ Empty = Empty
set (StepL:t) val (Node v l r) = Node v (set t val l) r
set (StepR:t) val (Node v l r) = Node v l (set t val r)
...
search val n | contains val n= Just $ search' val n
             | otherwise = Nothing
  where search' v (Node v' l r) | v == v' = []
                        | contains val l = StepL:(search' val l)
                        | contains val r = StepR:(search' val r)
        search' _ Empty = error "should never be epmty here"
        contains v t = not . allValues (/= v) $ t
