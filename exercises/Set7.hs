...
member a (Set s) = any id . map equalsa $ s where
  equalsa = (==) a

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add a (Set [])                = Set [a]
-- why does the above give an error if we use `emptyset `
add a (Set (h:t)) | h<a       = case (add a (Set(t))) of
                      Set s' -> Set (h:s')
                  | h>a       = Set (a:h:t)
                  | otherwise = Set (h:t)
...
data State = Start | Error | Finished | HasEggs | HasSugar | HasFlour | HasFlourAndSugar | Mixed 
  deriving (Eq,Show)
...
average l = (total/count) where
  (total, count) = foldl (\(tot, cnt) el -> (tot+el, cnt +1)) (0,0) l
...
reverseNonEmpty (h:|[]) = h:|[]
reverseNonEmpty (h:|t) = case  reverse (h:t) of
  h':t' -> h':|t'
  -- missing case matches, this should be avoidable...
...
operation "*" lhs rhs = Multiplication lhs rhs
operation "+" lhs rhs = Addition lhs rhs

evaluate :: Arithmetic -> Integer
evaluate (Literal i) = i
evaluate (Addition lhs rhs) = evaluate lhs + evaluate rhs
evaluate (Multiplication lhs rhs) = evaluate lhs * evaluate rhs

render :: Arithmetic -> String
render (Literal i) = show i
render (Addition lhs rhs) = "("++render lhs++"+"++render rhs++")"
render (Multiplication lhs rhs) = "("++render lhs++"*"++render rhs++")"
