...
instance Eq Name where
  (==) (Name a1) (Name a2) = map toLower a1 == map toLower a2
...
instance Eq a => Eq (List a) where
  (==) Empty Empty = True
  (==) (LNode h1 t1) (LNode h2 t2) = h1 == h2 && t1 == t2
  (==) _ _ = False
...
instance Eq RationalNumber where
  RationalNumber p1 q1 == RationalNumber p2 q2 = (p1 * q2) == (p2 *q1)
...
simplify (RationalNumber p q) = RationalNumber (p `div` qg) (q `div` qg) where
  qg = gcd p q
