...
prettyPrint sz coords = concat . map (line sz coords) $ [1..sz]


-- get the string for a given line 
line :: Size -> [Coord] -> Int -> String
line w m i = map char_for_j [1..w] ++ "\n" where
  char_for_j j = if elem (i,j) m then 'Q' else '.'
--------------------------------------------------------------------------------
...
danger candide queens = any (cankill candide) $ queens where
  cankill :: Candidate -> Coord -> Bool
  cankill c q =  any (\f -> f c q) [sameRow, sameCol, sameDiag, sameAntidiag]
...
prettyPrint2  sz coords = concat . map (line2 sz coords) $ [1..sz]


-- get the string for a given line 
line2 :: Size -> [Coord] -> Int -> String
line2 w m i = map char_for_j [1..w] ++ "\n" where
  char_for_j j | elem (i,j) m = 'Q'
               | danger (i,j) m = '#'
               | otherwise ='.'
...
fixFirst n s = case s of
  h:t -> case h of
    (i,j) -> if j >n then Nothing
      else  if not $ danger (i,j) t then (Just (h:t))
      else fixFirst n (nextCol h:t)
  []-> Nothing
...
finish n s | length s == n+1  = tail s
