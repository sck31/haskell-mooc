...
totalPrice entry = case entry of
  MkShoppingEntry _ cost units -> (fromIntegral units) * cost
...
buyOneMore entry = case entry of
  MkShoppingEntry name cost units -> MkShoppingEntry name cost (units + 1)
...
setName name p = case p of
  MkPerson _ age -> MkPerson name age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = case p of
  MkPerson name _ -> MkPerson name age
...
up p = case p of
  ...
  MkPosition x y -> MkPosition x (y+1)


-- right increases the x value of a position by one
right :: Position -> Position
right p = case p of
  MkPosition x y -> MkPosition (x+1) y
...
rgb col = case col of
  ...
  Green-> [0,1,0]
  Blue -> [0,0,1]
  Mix col1 col2 -> case (rgb col1 , rgb col2) of
    ([a1,b1,c1], [a2,b2,c2]) -> [(a1+a2)/2,(b1+b2)/2,(c1+c2)/2]
  Invert col1 ->  case rgb col1 of
...
toList Empty = []
toList (Pair kk vv tl) = (kk, vv):(toList tl)

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((kk,vv):t) =  Pair kk vv $ fromList  t
...
fromNat n = case n of
  Zero -> 0
  PlusOne nn -> 1 + fromNat nn

toNat :: Int -> Maybe Nat
toNat z = case z of
  0 -> Just $ Zero
  n | n > 0 -> case toNat $ n - 1 of
        Nothing -> Nothing
        Just k -> Just $ PlusOne k
    | otherwise -> Nothing
...
toBin' :: Int -> Bin
toBin' 0 = End
toBin' n = case (n `mod` 2, n `div` 2) of
  (1, m) -> I (toBin' m)
  (0, m) -> O (toBin' m)
