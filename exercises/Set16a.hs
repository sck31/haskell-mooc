...
inputInOutput input output =
  forAll (elements input) (\x -> elem x outvals) where
  outvals = map fst output
...
outputInInput input output =
  forAll (elements output) (\x -> snd x == count (fst x) input) where
  count _ [] = 0
...
genList = do
  k <- choose (3,5)
  let chars = elements [0..10]
  fmap sort $ vectorOf k chars
...
instance Arbitrary Arg where
  arbitrary = do
    n <- elements [0..10]
    c <- elements ['a','b','c','x','y','z']
    elements [Number n, Variable c]
